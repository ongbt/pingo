...
describe('HomePage', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(useMutation).mockReturnValue(mockSeedMutation);
  });
  ...
  it('renders guest state correctly', () => {
    ...
    const profileLink = screen.getAllByRole('link', { name: /account_circle\s*Profile/i });
  ...
  it('renders authenticated state correctly with avatar', () => {
    vi.mocked(usePingoAuth).mockReturnValue({
      ...
      profile: { id: '1', nickname: 'Tester', avatar_url: 'https://example.com/avatar.png' },
      isLoading: false,
      signOut: vi.fn(),
  ...
  it('renders authenticated state correctly with initial', () => {
    vi.mocked(usePingoAuth).mockReturnValue({
      user: { email: 'test@example.com' } as any,
      profile: { id: '1', nickname: 'Tester', avatar_url: null },
      isLoading: false,
      signOut: vi.fn(),
      isAuthenticated: true
    });
